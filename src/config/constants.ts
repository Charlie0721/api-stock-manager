//# Conexion a la base de datos
export const HOST = "localhost";
export const USER = "root";
export const PASSWORD = "conexion1234";
export const DATABASE = "empresa002";

//# ruta para guardar los archivos texto del recolector
export const DIRECTORYTOSAVE = "/Users/Desarrollo Carlos/Documents/";

//# Fecha de finalizacion de la suscripción
export const SUBSCRIPTION_END_DATE = "February 28 2025 12:00:00 GMT-0500";

//#Email de envio
export const USER_EMAIL = "carloscasasramirez@gmail.com";
export const EMAIL_PASSWORD = "jxbqxndyawoppdsc";
//#Correo desde el cual se envia la orden
export const SENDER_MAIL = "carloscasasramirez@gmail.com";
